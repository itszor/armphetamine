...
CC	= gcc
CFLAGS	= -g -pipe -Wall -O6 -mcpu=k6 -march=k6 -DQUICKWRITE -DVERSION=\"0.2\"
INCLUDE	= -I/usr/include -I/usr/include/readline -I.

LDFLAGS= -g

OBJ =	allocate.o analyse.o block.o cnew.o codegen.o decode.o disassemble.o \
	execute.o fakesys.o generators.o hash.o list.o loadaout.o machine.o \
	main.o memory.o nativesupport.o pqueue.o pseudo.o pseudodism.o \
	registers.o riscos.o x86asm.o x86dism.o rtasm.o rtasm_fns.o flush.o \
	pheta.o phetadism.o processor.o vidc20.o iomd.o asmalu.o debug.o \
	bset.o

SRC =	allocate.c analyse.c asmalutab.c block.c cnew.c codegen.c decode.c \
	disassemble.c execute.c fakesys.c flush.c generators.c hash.c iomd.c \
	list.c loadaout.c machine.c main.c memory.c nativesupport.c pheta.c \
	phetadism.c pqueue.c processor.c pseudo.c pseudodism.c registers.c \
	riscos.c rtasm.c vidc20.c x86asm.c x86dism.c rtasm.c flush.c pheta.c \
	phetadism.c vidc20.c iomd.c debug.c bset.c

LIBS = -lm -lreadline -lhistory -lncurses
