...
foreach (@inteldef)
{
  ...
  {
    chop;
    ($inst,$info,$dest,$destflag,$mang,$src1,$src2,$srcflag,$types)
      = split(/\|/);
    $inst =~ s/\s//g;
    $narg = 0;
    $narg++ if $dest =~ m/r/;
    ...
    {
      @type = split(//, $type);
      $fname = ucfirst lc $inst;
      foreach $part (@type)
      {
        $fname = $fname.'Reg'   if $part eq 'r';
        $fname = $fname.'Imm'   if $part eq 'k';
        $fname = $fname.'Mem'   if $part eq 'm';
        $fname = $fname.'Cl'    if $part eq 'C';
        $fname = $fname.'Near'  if $part eq 'n';
        $fname = $fname.'Short' if $part eq 's';
        $fname = $fname.'Byte'  if $part eq 'b';
        $fname = $fname.'Word'  if $part eq 'w';
        $fname = $fname.'DWord' if $part eq 'd';
      }
    #  print "$fname\n";
    }
  }
